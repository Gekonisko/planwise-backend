...
// that fires is reported as a RiskFeatureContribution so the explanation drawer can show its own math.
internal static class RiskScorer
{
    ...
    public sealed record ScoreResult(decimal Probability, int DayImpact, string Reason, IReadOnlyList<RiskFeatureContribution> Features);
    ...
    public static ScoreResult Score(TaskInsightSummary task, IReadOnlyDictionary<Guid, TaskInsightSummary> allTasksById, DateOnly today)
    {
        ...
        if (task.DueDate is DateOnly dueDate)
        {
            ...
            {
                score += OverdueWeight;
                features.Add(new RiskFeatureContribution("Overdue", OverdueWeight, $"Due date was {-daysUntilDue} day(s) ago"));
            }
            else if (daysUntilDue <= DueSoonDays)
            {
                score += DueSoonWeight;
                features.Add(new RiskFeatureContribution("DueSoon", DueSoonWeight, $"Due in {daysUntilDue} day(s)"));
            }
        }
        ...
        if (openBlockers > 0)
        {
            ...
            score += weight;
            features.Add(new RiskFeatureContribution("BlockedByOpenDependencies", weight, $"{openBlockers} predecessor task(s) not yet done"));
        }

        if (task.AssigneeId is null)
        {
            score += UnassignedWeight;
            features.Add(new RiskFeatureContribution("Unassigned", UnassignedWeight, "No assignee"));
        }
        ...
        if (task.Points is int points && points >= LargeScopeThresholdPoints)
        {
            ...
            score += weight;
            features.Add(new RiskFeatureContribution("LargeScope", weight, $"{points} points"));
        }
    ...
    private static string BuildReason(List<RiskFeatureContribution> features)
    {
        if (features.Count == 0)
        {

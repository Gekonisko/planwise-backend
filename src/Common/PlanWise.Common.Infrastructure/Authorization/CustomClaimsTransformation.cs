...
using PlanWise.Common.Application.Authorization;
...
namespace PlanWise.Common.Infrastructure.Authorization;
...
internal sealed class CustomClaimsTransformation(IServiceScopeFactory serviceScopeFactory) : IClaimsTransformation
{
    public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
    {
        ...
        if (result.IsFailure)
        {
            throw new PlanWiseException(nameof(IPermissionService.GetUserPermissionsAsync), result.Error);
        }
